...
router.put('/user/metrics', auth, async (req, res) => {
    try {
        ...
        const existingMetrics = user.metrics ? user.metrics.toObject() : {};
        const updatedMetrics = { ...existingMetrics, ...req.body };

        // Recalculate Daily Calories
        let bmr = 0;
        const weight = parseFloat(updatedMetrics.weight) || 70;
        const height = parseFloat(updatedMetrics.height) || 170;
        const age = updatedMetrics.dob ? (new Date().getFullYear() - new Date(updatedMetrics.dob).getFullYear()) : 25;

        if (updatedMetrics.gender === 'Female') {
        ...
        switch (updatedMetrics.activityLevel) {
        ...
        if (updatedMetrics.weeklyGoal) {
            if (updatedMetrics.weeklyGoal.includes('Lose 0.25')) tdee -= 250;
            else if (updatedMetrics.weeklyGoal.includes('Lose 0.5')) tdee -= 500;
            else if (updatedMetrics.weeklyGoal.includes('Lose 1')) tdee -= 1000;
            else if (updatedMetrics.weeklyGoal.includes('Gain 0.25')) tdee += 250;
            else if (updatedMetrics.weeklyGoal.includes('Gain 0.5')) tdee += 500;
        }

        updatedMetrics.dailyCalories = Math.round(tdee);
        user.metrics = updatedMetrics;
